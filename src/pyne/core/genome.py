...
from .._cpp.genotype import CPPNData
...
class Genome(CPPNData):
    ...
    def _add_node(self, func: str) -> int:
        ...
        self.nodes.append(CPPNData.Node(nid, func))
        self.nextNodeID += 1
        return nid
    ...
    def _add_link(self, src: int, dst: int, weight: float) -> int:
        ...
        """
        lid = self.nextLinkID
        self.links.append(CPPNData.Link(lid, src, dst, weight))
        self.nextLinkID += 1
        return lid

    def __random_add_node(self, rng: Random) -> None:
        i = rng.randrange(0, len(self.links))
        link: CPPNData.Link = self.links[i]
    ...
    def random(rng: Random):
        ...
        for i in range(CPPNData.INPUTS):
            for j in range(CPPNData.OUTPUTS):
                if rng.random() < .5:
                    g._add_link(i, j + CPPNData.INPUTS,
    ...
    def to_dot(self, path: str, ext: str = "pdf", title: str = None, debug=None) -> str:
        ...
        def png_file(func):
            p = str((files('pyne') / f"core/functions/{func}.png").resolve())
            print(p)
            return p
