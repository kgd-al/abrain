...
void init_config (py::module_ &m) {
  ...
  auto mr = py::bind_map<Config::MutationRates>(m, "MutationRates");
...
#define CLASS Config
  ...
  config.doc() = "C++/Python configuration values for the PyNE library";


#undef CLASS
#define CLASS Config::FBounds
  auto fb = py::class_<Config::FBounds>(m, "FBounds")
      .def(py::init<>())
  ...
  fb.def_static("ccpParseString", [] (const std::string &s) {
    Config::FBounds b {};
  ...
  mr.def_static("ccpParseString", [] (const std::string &s) {
